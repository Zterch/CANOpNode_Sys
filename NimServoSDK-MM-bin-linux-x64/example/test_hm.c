...
int main(int argc, char* argv[]) 
{
    ...
    { 
        ...
        nRe = Nim_set_homeType(hMaster, nAddrs[nIndex], 37);
        nRe = Nim_set_workMode(hMaster, nAddrs[nIndex], SERVO_HM_MODE, 1);
        MySleepMS(50);      // 必要延时(Nim_set_workMode后)
        nRe = Nim_set_goHome_velocity(hMaster, nAddrs[nIndex], 10.0, 0.5);
        nRe = Nim_set_goHome_accel(hMaster, nAddrs[nIndex], 5.0);
        nRe = Nim_set_homeOffset(hMaster, nAddrs[nIndex], 0.0);
        MySleepMS(50);
        nRe = Nim_power_on(hMaster, nAddrs[nIndex], 1);
    }
    MySleepMS(200);          // 必要延时(Nim_power_on后)
    // 控制电机在HM模式下运行
    nRe = Nim_goHome(hMaster, nAddrs[0], 1);
    printf("********************go home*********************\n");
    double fPos = 0.0;
    ...
    do
    {
        ...
        if (0 == Nim_get_currentPosition(hMaster, nAddrs[0], &fPos, 0)
                && 0 == Nim_get_currentVelocity(hMaster, nAddrs[0], &fVelocity, 0)
