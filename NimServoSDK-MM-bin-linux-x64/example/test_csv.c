...
int main(int argc, char* argv[])
{
    ...
    for (int nIndex = 0; nIndex < nSlaveCount; nIndex++)
    {
        ...
        nRe = Nim_set_workMode(hMaster, nAddrs[nIndex], SERVO_CSV_MODE, 1);
        MySleepMS(50);      // 必要延时(Nim_set_workMode后)
        nRe = Nim_set_ipPosition(hMaster, nAddrs[nIndex], 0, 1);
        nRe = Nim_power_on(hMaster, nAddrs[nIndex], 1);
    }
    MySleepMS(200);          // 必要延时(Nim_power_on后)

    printf("********************run Follow the sin curve*********************\n");
    double fPos = 0.0;
    double fSpeed = 0.0;
    for (i=0; i<800; i++)
    {
        double speed = 2 * sin(i*3.14159265/200.0);
        Nim_set_targetVelocity(hMaster, nAddrs[0], speed, 0); 
        MySleepMS(10);
