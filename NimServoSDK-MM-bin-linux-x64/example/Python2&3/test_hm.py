...
if res == 0:
    ...
    if nCurAddr > 0:
        ...
        Nim_set_homeType(hMaster, nCurAddr, 37)
        [nRes, nType] = Nim_get_homeType(hMaster, nCurAddr)
        print("exec Nim_get_homeType: %d" % (nType))
        Nim_set_workMode(hMaster, nCurAddr, ServoWorkMode.SERVO_HM_MODE, 1)
        time.sleep(0.05)     # 必要延时(Nim_set_workMode)
        Nim_set_goHome_velocity(hMaster, nCurAddr, 3.0, 0.5)
        Nim_set_goHome_accel(hMaster, nCurAddr, 2.0)
        Nim_set_homeOffset(hMaster, nCurAddr, 0.0)
        Nim_power_on(hMaster, nCurAddr, 1)
        time.sleep(0.2)      # 必要延时(Nim_power_on后)
        
        #控制电机在HM模式下运行
        Nim_goHome(hMaster, nCurAddr, 1);
        print("********************go home*********************")
        Nim_set_vmTargetSpeed(hMaster, nCurAddr, 200, 1)
        while True:
            time.sleep(0.1)
            [nRes, sw] = Nim_get_statusWord(hMaster, nCurAddr, 0)
            [nRes, pos] = Nim_get_currentPosition(hMaster, nCurAddr, 0)
            [nRes, Speed] = Nim_get_currentVelocity(hMaster, nCurAddr, 0)
            print("result: %d, status_word: %d, position: %f, velocity: %f" % (nRes, sw, pos, Speed))
            if (sw & 0x1000) != 0:
                break
