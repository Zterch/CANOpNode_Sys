...
if res == 0:
    ...
    if nCurAddr > 0:
        ...
        Nim_set_workMode(hMaster, nCurAddr, ServoWorkMode.SERVO_PV_MODE, 1)
        time.sleep(0.05)     # 必要延时(Nim_set_workMode)
        Nim_set_profileDecel(hMaster, nCurAddr, 5.5)
        Nim_set_profileAccel(hMaster, nCurAddr, 5.5);
        Nim_power_on(hMaster, nCurAddr, 1)
        time.sleep(0.2)
        print("********************run forward*********************")
        Nim_forward(hMaster, nCurAddr, -5.0, 0)
        while True:
            time.sleep(0.1)
        ...
        print("*******************run backward*********************")
        Nim_backward(hMaster, nCurAddr, 5.0, 1)
        while True:
            time.sleep(0.1)
            [nRes, sw] = Nim_get_statusWord(hMaster, nCurAddr, 0)
            [nRes, pos] = Nim_get_currentPosition(hMaster, nCurAddr, 0)
        ...
        print("*********************run stop***********************")
        Nim_forward(hMaster, nCurAddr, 0.0, 1)
        time.sleep(2.0)
        while True:
            time.sleep(0.1)
            [nRes, sw] = Nim_get_statusWord(hMaster, nCurAddr, 0)
