...
if res == 0:
    ...
    if nCurAddr > 0:
        ...
        Nim_set_workMode(hMaster, nCurAddr, ServoWorkMode.SERVO_VM_MODE, 1)
        time.sleep(0.05)     # 必要延时(Nim_set_workMode)
        Nim_set_vmSpeedLimit(hMaster, nCurAddr, 0, 500)
        Nim_set_vmAccel(hMaster, nCurAddr, 100, 1)
        Nim_set_vmDecel(hMaster, nCurAddr, 100, 1) 
        Nim_power_on(hMaster, nCurAddr, 1)
        time.sleep(0.2)
        
        Nim_set_controlWord(hMaster, nCurAddr, 0x7F, 1)
        print("********************run forward*********************")
        Nim_set_vmTargetSpeed(hMaster, nCurAddr, 200, 1)
        while True:
            time.sleep(0.1)
            [nRes, sw] = Nim_get_statusWord(hMaster, nCurAddr, 0)
            [nRes, vmSpeed] = Nim_get_currentVelocity2(hMaster, nCurAddr, 0)
            print("result: %d, status_word: %d, velocity: %f" % (nRes, sw, vmSpeed))
            if (sw & 0x400) != 0:
                break
        time.sleep(5.0)
        ...
        print("**********************run stop***********************")
        Nim_set_vmTargetSpeed(hMaster, nCurAddr, 0, 1)
        time.sleep(2.0)
        while True:
            time.sleep(0.1)
            [nRes, sw] = Nim_get_statusWord(hMaster, nCurAddr, 0)
