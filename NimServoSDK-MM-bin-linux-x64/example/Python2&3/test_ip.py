...
if res == 0:
    ...
    if nCurAddr > 0:
        ...
        Nim_set_workMode(hMaster, nCurAddr, ServoWorkMode.SERVO_IP_MODE, 1)
        time.sleep(0.05)     # 必要延时(Nim_set_workMode)
        # 设置插补周期为50ms 与conn_str中" \"PDOIntervalMS\": 50参数保持一致
        nRe = Nim_set_ipPeriod(hMaster, nCurAddr, 50);
        [res, nVal] = Nim_get_ipPeriod(hMaster, nCurAddr)
        print("Nim_get_ipPeriod %d\n" % nVal)
        [res, startPos] = Nim_get_currentPosition(hMaster, nCurAddr, 1)
        Nim_set_ipPosition(hMaster, nCurAddr, startPos, 1)
        Nim_power_on(hMaster, nCurAddr, 1)
        time.sleep(0.2)      # 必要延时(Nim_power_on后)
        
        Nim_set_controlWord(hMaster, nCurAddr, 0x1F, 1)
        print("********************move ip*********************\n")
        t = 0.0
        while True:
            fPosition = 2.0 * (math.sin(2.0*math.pi/5.0*t - math.pi/2.0) + 1.0);
            Nim_set_ipPosition(hMaster, nCurAddr, startPos+fPosition, 0)
            time.sleep(0.05)
            t = t + 0.05
