...
int main(int argc, char* argv[])
{
    ...
    for (int nIndex = 0; nIndex < nSlaveCount; nIndex++)
    {
        ...
        nRe = Nim_set_workMode(hMaster, nAddrs[nIndex], SERVO_PT_MODE, 1);
        MySleepMS(50);      // 必要延时(Nim_set_workMode后)
        nRe = Nim_set_PT_SpeedLimit(hMaster, nAddrs[nIndex], 500, 500);
        unsigned short FwrSpeedLimit = 0;
        unsigned short BwrSpeedLimit = 0;
        unsigned int   torqueRamp = 0;
        Nim_get_PT_SpeedLimit(hMaster, nAddrs[nIndex], &FwrSpeedLimit, &BwrSpeedLimit);
        Nim_set_PT_TorqueRamp(hMaster, nAddrs[nIndex], 0);
        Nim_get_PT_TorqueRamp(hMaster, nAddrs[nIndex], &torqueRamp);
        Nim_set_targetTorque(hMaster, nAddrs[nIndex], 200, 1);
        printf("FwrSpeedLimit = %d BwrSpeedLimit = %d torqueRamp = %d\n", FwrSpeedLimit, BwrSpeedLimit, torqueRamp);
    ...
    {
        MySleepMS(100);
        if (0 == Nim_get_currentTorque(hMaster, nAddrs[0], &torque, 1))
        {
            printf("Current torque is %d\t\r", torque);
			fflush(stdout);
        }
    }
    for (int nIndex = 0; nIndex < nSlaveCount; nIndex++) 
