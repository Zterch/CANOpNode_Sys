...
int main(int argc, char* argv[])
{
    ...
    for (int nIndex = 0; nIndex < nSlaveCount; nIndex++)
    {
        ...
        nRe = Nim_set_workMode(hMaster, nAddrs[nIndex], SERVO_PV_MODE, 1);
        MySleepMS(50);      // 必要延时(Nim_set_workMode后)
        nRe = Nim_set_profileAccel(hMaster, nAddrs[nIndex], 5.5);
        nRe = Nim_set_profileDecel(hMaster, nAddrs[nIndex], 5.5);
        nRe = Nim_power_on(hMaster, nAddrs[nIndex], 1);
    } 
    MySleepMS(200);          // 必要延时(Nim_power_on后)
    printf("********************run forward*********************\n");
    nRe = Nim_forward(hMaster, nAddrs[0], 3.0, 1);
    double fVelocity = 0.0;
    unsigned short statusWord = 0;
    do
    {
        MySleepMS(50);
        if (0 == Nim_get_currentVelocity(hMaster, nAddrs[0], &fVelocity, 0)
            && 0 == Nim_get_statusWord(hMaster, nAddrs[0], &statusWord, 0))
            printf("statusword = %04x \t currentVelocity = %f\t\r", statusWord, fVelocity);
			fflush(stdout);
    ...
    printf("********************run backward*********************\n");
    nRe = Nim_backward(hMaster, nAddrs[0], -3.0, 1);
    do
    {
        MySleepMS(50);
        if (0 == Nim_get_currentVelocity(hMaster, nAddrs[0], &fVelocity, 0)
    ...
    printf("\n");
    MySleepMS(4000);
    nRe = Nim_backward(hMaster, nAddrs[0], 0.0, 1);
