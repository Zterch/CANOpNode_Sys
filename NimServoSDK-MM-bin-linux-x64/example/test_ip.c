...
int main(int argc, char* argv[])
{
    ...
    case 0:     //CANopen
        ...
        case 1001:      //NiMotion USB-CAN Device
            sprintf(conn_str, "{\"DevType\": \"%s\", \"DevIndex\": 0, \"Baudrate\": 8,"\
                              " \"PDOIntervalMS\": 20, \"SyncIntervalMS\": 20}", argv[2]);
            break;
    ...
    for (int nIndex = 0; nIndex < nSlaveCount; nIndex++)
    {
        ...
        nRe = Nim_set_workMode(hMaster, nAddrs[nIndex], SERVO_IP_MODE, 1);
        MySleepMS(50);      // 必要延时(Nim_set_workMode后)
        // 设置插补周期为20ms 与conn_str中" \"PDOIntervalMS\": 20参数保持一致
        nRe = Nim_set_ipPeriod(hMaster, nAddrs[nIndex], 20); 
        printf("Nim_set_ipPeriod %d ret:%d\n", 20, nRe);
        unsigned int val;
        nRe = Nim_get_ipPeriod(hMaster, nAddrs[nIndex], &val);
        printf("Nim_get_ipPeriod %d ret:%d\n", val, nRe);
        nRe = Nim_get_currentPosition(hMaster, nAddrs[nIndex], &fStartPos[nIndex], 1);
        nRe = Nim_set_ipPosition(hMaster, nAddrs[nIndex], fStartPos[nIndex], 1);
        nRe = Nim_power_on(hMaster, nAddrs[nIndex], 1);
    }
    MySleepMS(200);          // 必要延时(Nim_power_on后) 

    // 控制字发0x1F控制电机在IP模式下运行
    nRe = Nim_set_controlWord(hMaster, nAddrs[0], 0x1F, 0);
    printf("********************run Follow the cos curve*********************\n");
    double fPos = 0.0;
    double fSpeed = 0.0;
    for (i=0; i<800; i++)
    {
        double pos = 5 * (cos(i*3.14159265/200.0) - 1.0);
        for (int nIndex = 0; nIndex < nSlaveCount; nIndex++)
        {
            Nim_set_ipPosition(hMaster, nAddrs[nIndex], fStartPos[nIndex]+pos, 0);
        }
        MySleepMS(20);  // 设置插补周期为20ms 与conn_str中" \"PDOIntervalMS\": 20参数保持一致
        if (0 == Nim_get_currentPosition(hMaster, nAddrs[0], &fPos, 0)
            && 0 == Nim_get_currentVelocity(hMaster, nAddrs[0], &fSpeed, 0))
            printf("Current pos is %f\tCurrent speed is %f\t\r", fPos, fSpeed);
			fflush(stdout);
    }
    printf("\n");
    ...
    for (int nIndex = 0; nIndex < nSlaveCount; nIndex++)
    {
        nRe = Nim_power_off(hMaster, nAddrs[nIndex], 0);
    }
    MySleepMS(50);          // 必要延时(Nim_power_off后) 
