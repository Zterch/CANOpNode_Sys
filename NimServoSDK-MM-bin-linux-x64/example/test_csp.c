...
int main(int argc, char* argv[])
{
    ...
    for (int nIndex = 0; nIndex < nSlaveCount; nIndex++)
    {
        ...
        nRe = Nim_set_workMode(hMaster, nAddrs[nIndex], SERVO_CSP_MODE, 1);
        MySleepMS(50);      // 必要延时(Nim_set_workMode后)
        nRe = Nim_get_currentPosition(hMaster, nAddrs[nIndex], &fStartPos[nIndex], 1);
        nRe = Nim_set_targetPosition(hMaster, nAddrs[nIndex], fStartPos[nIndex], 1);
    ...
    for (i=0; i<800; i++)
    {
        double pos = 1 * (cos(i*3.14159265/200.0) - 1.0);
        for (int nIndex = 0; nIndex < nSlaveCount; nIndex++)
        {
            Nim_set_targetPosition(hMaster, nAddrs[nIndex], fStartPos[nIndex]+pos, 0);
        }
        MySleepMS(10);
        if (0 == Nim_get_currentPosition(hMaster, nAddrs[0], &fPos, 0) 
            && 0 == Nim_get_currentVelocity2(hMaster, nAddrs[0], &fSpeed, 0))
        {
